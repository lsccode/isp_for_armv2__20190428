...
static interp_2d_point_t init_interp_2d_point( awb_calibration_data_t *p_cali_data, int32_t i, int32_t j,
        const int16_t *p_table, const int16_t *posx, const int16_t *posy )
{
...
static int32_t AWB_mesh( awb_calibration_data_t *p_cali_data, uint32_t rg, uint32_t bg,
                         int16_t *posx, int16_t *posy, const int16_t *p_table, uint8_t flag )
{
...
int32_t AWB_mesh_LUT( awb_calibration_data_t *p_cali_data,
                      int32_t rg, int32_t bg,
                      int16_t minx, int16_t maxx,
                      int16_t stepx,
                      int16_t miny, int16_t maxy,
                      int16_t stepy,
                      const int16_t *p_table )
{
    uint32_t lut_length = p_cali_data->cali_rg_pos_len;
...
static int32_t mesh_AWB_getKnownSourceLight_weight_LUT( awb_calibration_data_t *p_cali_data,
        uint16_t rg, uint16_t bg,
        int32_t light_rg, int32_t light_bg )
{
    uint32_t lut_length = p_cali_data->cali_rg_pos_len;
    ...
    if ( valid_points ) {
        weight = AWB_mesh_LUT( p_cali_data, (int32_t)rg, (int32_t)bg, MinX, MaxX,
                               ( MaxX - MinX ) / ( lut_length - 1 ), MinY, MaxY,
                               ( MaxY - MinY ) / ( lut_length - 1 ),
                               (int16_t *)p_cali_data->cali_mesh_ls_weight ); // division by zero is checked
...
//Get average (rg&bg)
static void awb_calc_avg_weighted_gr_gb_mesh( awb_acamera_core_obj_t *p_awb_core_obj,
        awb_stats_data_t *stats, awb_input_data_t *input )
{
    uint64_t sum = 0;
    uint32_t npcH = 0;
    uint32_t npcL = 0;
...
//
    ...
    if ( p_acamera_input->misc_info.cur_exposure_log2 < p_cali_data->cali_evtolux_ev_lut[0] ) {
        lux = p_cali_data->cali_evtolux_lux_lut[0];
                ...
                p_cali_data->cali_evtolux_ev_lut[p_cali_data->cali_evtolux_ev_lut_len - 1] ) {
        ...
        lux = interpl( p_acamera_input->misc_info.cur_exposure_log2,
                       p_cali_data->cali_evtolux_ev_lut[_i - 1],
                       p_cali_data->cali_evtolux_lux_lut[_i - 1],
                       p_cali_data->cali_evtolux_ev_lut[_i],
                       p_cali_data->cali_evtolux_lux_lut[_i] );
    }
#endif
    ...
    for ( _i = 0; _i < stats->zones_size; _i++ ) {
        ...
        if ( valid_zone ) {
            ...
            weight = AWB_mesh( p_cali_data, (uint32_t)rg, (uint32_t)bg,
                               (int16_t *)p_cali_data->cali_rg_pos, (int16_t *)p_cali_data->cali_bg_pos,
                               (int16_t *)p_cali_data->cali_mesh_rgbg_weight, 0 );

            // Extra light source probabilities
            for ( _j = 0; _j < p_acamera_input->cali_data.cali_light_src_len; _j++ ) {
                weight_ls = mesh_AWB_getKnownSourceLight_weight_LUT( p_cali_data, rg, bg,
                            _calibration_light_src[_j][0], _calibration_light_src[_j][1] );
    ...
    if ( enable_awb_mix_light && lux > lux_low && lux < lux_high &&
        ...
        for ( _i = 0; _i < stats->zones_size; _i++ ) {
            ...
            rg_weight = luts_fetch( rg, rgHigh_LUT_min, rgHigh_LUT_max, 1,
                                    (int16_t *)rghigh_lut, array_size( rghigh_lut ) );
    ...
    } else if ( ( ( npcHmean >= npcLmean ) && ( npcHmean - npcLmean < 15 ) ) ||
                ( ( npcHmean < npcLmean ) && ( npcLmean - npcHmean < 15 ) ) ) {
        ...
        if ( lux < 10 ) {
            ...
            for ( _i = 0; _i < stats->zones_size; _i++ ) {
                if ( mvalid[_i] &&
                        stats->awb_zones[_i].rg >=
                        p_cali_data->cali_ct_rg_pos_calc[p_cali_data->cali_ct65pos[0]] &&
                        stats->awb_zones[_i].bg <=
                        p_cali_data->cali_ct_bg_pos_calc[p_cali_data->cali_ct65pos[0]] ) { // division by zero is checked
        ...
        if ( lux > p_cali_data->cali_sky_lux_th[0] && avg_BG > 246 ) {
            ...
            for ( _j = 25; _j <= 65; _j += 20 ) {
                ...
                sum = 0;
                for ( _i = 0; _i < stats->zones_size; _i++ ) {
                    if ( ( stats->awb_zones[_i].rg >= sky_cut_high ) &&
                            ( stats->awb_zones[_i].rg <= sky_cut_low ) &&
                            ( stats->awb_zones[_i].bg <= sky_cut_high_bg ) ) { // division by zero is checked
                        sky_zones[_i] = 1;
                    } else {
                        sky_zones[_i] = 0;
                    }
    ...
    if ( !p_acamera_input->misc_info.global_manual_awb ) {
        ...
        if ( p_awb_core_obj->avg_GR && p_awb_core_obj->avg_GB ) {
            ...
            temp_cal = AWB_mesh( p_cali_data,
                                 ( uint32_t )( U16_MAX / p_awb_core_obj->avg_GR ),
                                 ( uint32_t )( U16_MAX / p_awb_core_obj->avg_GB ),
                                 (int16_t *)p_cali_data->cali_rg_pos,
                                 (int16_t *)p_cali_data->cali_bg_pos,
                                 (int16_t *)p_cali_data->cali_mesh_color_temperature,
                                 1 ); // division by zero is checked
            if ( temp_cal )
        ...
        temp_cal = AWB_mesh( p_cali_data,
                             ( uint32_t )( p_awb_core_obj->rg_coef ? U16_MAX / p_awb_core_obj->rg_coef : U16_MAX ),
                             ( uint32_t )( p_awb_core_obj->bg_coef ? U16_MAX / p_awb_core_obj->bg_coef : U16_MAX ),
                             (int16_t *)p_cali_data->cali_rg_pos,
                             (int16_t *)p_cali_data->cali_bg_pos,
                             (int16_t *)p_cali_data->cali_mesh_color_temperature, 1 ); // division by zero is checked
        if ( temp_cal )
            temp_cal = 1000000 / temp_cal; // division by zero is checked
    ...
    */
    ...
    if ( temperature >= AWB_colour_preference[1] ) {
        ...
        if ( m == 0 ) {
            ...
            p_awb_core_obj->awb_warming[0] = ( m * ( temperature - AWB_colour_preference[0] ) +
                                               p_awb_core_obj->awb_warming_D75[0] ) >> 8;
        }
        ...
        if ( m == 0 ) {
            ...
            p_awb_core_obj->awb_warming[1] = ( m * ( temperature - AWB_colour_preference[0] ) +
                                               p_awb_core_obj->awb_warming_D75[1] ) >> 8;
        }
        //BLUE
        ...
        if ( m == 0 ) {
            ...
            p_awb_core_obj->awb_warming[2] = ( m * ( temperature - AWB_colour_preference[0] ) +
                                               p_awb_core_obj->awb_warming_D75[2] ) >> 8;
        }
