...
void awb_roi_update( awb_acamera_fsm_ptr_t p_fsm )
{
    ...
    uint8_t x_start = ( uint8_t )( ( ( ( p_fsm->roi >> 24 ) & 0xFF ) * horz_zones + 128 ) >> 8 );  // 0
    uint8_t x_end = ( uint8_t )( ( ( ( p_fsm->roi >> 8 ) & 0xFF ) * horz_zones + 128 ) >> 8 );     // 33
    uint8_t y_start = ( uint8_t )( ( ( ( p_fsm->roi >> 16 ) & 0xFF ) * vert_zones + 128 ) >> 8 );  // 0
    uint8_t y_end = ( uint8_t )( ( ( ( p_fsm->roi >> 0 ) & 0xFF ) * vert_zones + 128 ) >> 8 );     // 33
    uint8_t zone_size_x = x_end - x_start; // 33
    uint8_t zone_size_y = y_end - y_start; // 33
    uint32_t middle_x = zone_size_x * 256 / 2; // 4224
    uint32_t middle_y = zone_size_y * 256 / 2; // 4224
    ...
    uint16_t scale_x = ( horz_zones - 1 ) / ( len_zone_wght_hor > 0 ? len_zone_wght_hor : 1 ) + 1; // 2
    uint16_t scale_y = ( vert_zones - 1 ) / ( len_zone_wght_ver > 0 ? len_zone_wght_ver : 1 ) + 1; // 2

    uint16_t gaus_center_x = ( len_zone_wght_hor * 256 / 2 ) * scale_x; // 8448
    uint16_t gaus_center_y = ( len_zone_wght_ver * 256 / 2 ) * scale_y; // 8448
...
void awb_init( awb_acamera_fsm_t *p_fsm )
{
    ...
    if ( p_fsm->lib_handle ) {
        ...
        LOG( LOG_INFO, "AWB: init: %p, deinit: %p, proc: %p .", p_fsm->awb_alg_obj.init, p_fsm->awb_alg_obj.deinit, p_fsm->awb_alg_obj.proc );
    }

#else
    // Initialize awb algorithm object
    p_fsm->awb_alg_obj.init = awb_acamera_core_init;
    p_fsm->awb_alg_obj.proc = awb_acamera_core_proc;
    p_fsm->awb_alg_obj.deinit = awb_acamera_core_deinit;
#endif
    ...
    if ( MAX_AWB_ZONES < acamera_isp_metering_awb_nodes_used_horiz_read( p_fsm->cmn.isp_base ) *
...
void awb_read_statistics( awb_acamera_fsm_t *p_fsm )
{
    ...
    for ( _i = 0; _i < p_fsm->curr_AWB_ZONES; ++_i ) {
        ...
        // rg_coef is actually R_gain appiled to R Pixels.Since
        // we get (G*G_gain)/(R*R_gain) from HW,we multiply by the gain rg_coef to negate its effect.
...
void awb_process_light_source( awb_acamera_fsm_t *p_fsm )
{
    ...
    if ( p_fsm->light_source_detected == p_fsm->light_source_candidate ) {
        if ( ( p_fsm->light_source_candidate != ccm_info.light_source ) ||
                ( high_gain && ccm_info.light_source_ccm != AWB_LIGHT_SOURCE_UNKNOWN ) ||
                ( !high_gain && ccm_info.light_source_ccm == AWB_LIGHT_SOURCE_UNKNOWN ) ) {
            ++p_fsm->detect_light_source_frames_count;
            if ( p_fsm->detect_light_source_frames_count >=
                    p_fsm->switch_light_source_detect_frames_quantity &&
                    !ccm_info.light_source_change_frames_left ) {
...
void awb_normalise( awb_acamera_fsm_t *p_fsm )
{
    ...
    int32_t diff = ( ISP_INPUT_BITS << LOG2_GAIN_SHIFT ) -
                   acamera_log2_fixed_to_fixed( ( 1 << ISP_INPUT_BITS ) -
                           sensor_info.black_level, 0, LOG2_GAIN_SHIFT ) - min_wb;
    for ( i = 0; i < 4; ++i ) {
