    ...
    if ( exp <= p_ae_acamera_input->cali_data.ae_exp_corr_lut[0] )
        ...
        for ( i = 1; i < p_ae_acamera_input->cali_data.ae_exp_corr_lut_len; i++ ) {
            if ( exp < p_ae_acamera_input->cali_data.ae_exp_corr_lut[i] ) {
                correction = linear_interpolation_u8( p_ae_acamera_input->cali_data.ae_exp_corr_lut[i - 1], lut[i - 1], p_ae_acamera_input->cali_data.ae_exp_corr_lut[i], lut[i], exp );
                break;
            }
        }
    }
...
uint32_t ae_calculate_exposure_ratio( ae_acamera_core_obj_t *p_ae_core_obj, ae_stats_data_t *stats, ae_input_data_t *ae_input )
{
    ...
    const uint32_t max_exposure_ratio = adjacent_ratio_to_full( p_ae_acamera_input->misc_info.sensor_exp_number, p_ae_acamera_input->misc_info.global_max_exposure_ratio );

    if ( p_ae_acamera_input->misc_info.global_manual_exposure_ratio ) {
        exp_ratio = adjacent_ratio_to_full( p_ae_acamera_input->misc_info.sensor_exp_number, p_ae_acamera_input->misc_info.global_exposure_ratio );
    } else {
...
static void ae_calculate_target( ae_acamera_core_obj_t *p_ae_core_obj, ae_stats_data_t *stats, ae_input_data_t *ae_input )
{
    ...
    const int32_t hdr_target = acamera_calc_modulation_u16( log2_gain, p_ae_acamera_input->cali_data.ae_hdr_target, p_ae_acamera_input->cali_data.ae_hdr_target_len );
    ...
    p_ae_core_obj->error_log2 = acamera_log2_fixed_to_fixed( new_target, 0, LOG2_GAIN_SHIFT ) - acamera_log2_fixed_to_fixed( m1, 0, LOG2_GAIN_SHIFT ) +
                                ( ( ( int32_t )(ae_comp)-0x80 ) << ( LOG2_GAIN_SHIFT - 6 + 1 ) ) + ( ( (int32_t)exp_correction - 0x80 ) << ( LOG2_GAIN_SHIFT - 6 + 1 ) );
