...
void iridix_initialize( iridix_acamera_fsm_ptr_t p_fsm )
{
    ...
    if ( p_fsm->lib_handle ) {
        ...
        LOG( LOG_INFO, "Iridix: init: %p, deinit: %p, proc: %p .", p_fsm->iridix_alg_obj.init, p_fsm->iridix_alg_obj.deinit, p_fsm->iridix_alg_obj.proc );
    }

#else
    // Initialize iridix algorithm object
    p_fsm->iridix_alg_obj.init = iridix_acamera_core_init;
    p_fsm->iridix_alg_obj.proc = iridix_acamera_core_proc;
    p_fsm->iridix_alg_obj.deinit = iridix_acamera_core_deinit;
#endif
