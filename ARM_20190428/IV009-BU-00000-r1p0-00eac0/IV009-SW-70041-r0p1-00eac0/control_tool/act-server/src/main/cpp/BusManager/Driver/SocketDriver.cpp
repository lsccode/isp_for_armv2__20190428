        ...
        if (!rx_thread) {
            if (CATLError::GetLastError() == EATLErrorOk  && !dev_name.empty()) {
                ...
                if (semicolon_pos == string::npos) {    // no semicolon - UNIX socket
                    ...
                    WSADATA wsaData;
                    INT iStatus = WSAStartup(wVersionRequired, &wsaData);
                    if (iStatus != 0) {
                        printf("Could not initalise WinSock DLL: %x\n",
                                WSAGetLastError());
                        return FALSE;
                    }
                    else {
                        printf("Initialised WinSock DLL successfully.\n");
                    }
#endif
                    ...
                    if (soc > 0) { // works for both Windows and Linux
                        ...
                        if (!set_socket_timeout(soc, 100)) {
                            ...
                            // set IP address
                            inet_pton(AF_INET,
                                      dev_name.substr(0,semicolon_pos).c_str(),
                                      &service.sin_addr.s_addr);
                            #else
                            service.sin_addr.s_addr =
                            inet_addr(dev_name.substr(0,semicolon_pos).c_str());
                            #endif
